...
export function ProjectCard({ title, description, tags, image, demoUrl, repoUrl }: ProjectCardProps) {
  ...
  return (
    ...
    >
      <div
        className="relative h-full overflow-hidden rounded-xl bg-black/30 backdrop-blur-sm border border-zinc-700/50 transition-all duration-300 group-hover:border-purple-500/50"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        {/* Optionally, you can comment out or remove the gradient overlays below for a cleaner blend */}
